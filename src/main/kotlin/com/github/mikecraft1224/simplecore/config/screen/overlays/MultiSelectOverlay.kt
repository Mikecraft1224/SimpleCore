...
class MultiSelectOverlay(
    ...
    override fun render(ctx: DrawContext, mx: Int, my: Int) {
        ...
        for (i in opts.indices) {
            ...
            if (isHov) ctx.fill(px + 1, iy, px + popW - 1, iy + ITEM_H, C_SURFACE1)

            val cbX = px + CB_PAD
            val cbY = iy + (ITEM_H - CB_SIZE) / 2
            if (isChecked) {
                ctx.fill(cbX, cbY, cbX + CB_SIZE, cbY + CB_SIZE, accent)
            } else {
                ctx.fill(cbX,              cbY,              cbX + CB_SIZE,     cbY + 1,         C_SURFACE1)
                ctx.fill(cbX,              cbY + CB_SIZE - 1, cbX + CB_SIZE,    cbY + CB_SIZE,    C_SURFACE1)
                ctx.fill(cbX,              cbY,              cbX + 1,           cbY + CB_SIZE,    C_SURFACE1)
                ctx.fill(cbX + CB_SIZE - 1, cbY,             cbX + CB_SIZE,     cbY + CB_SIZE,    C_SURFACE1)
            }

            val labelX = cbX + CB_SIZE + 4
            val labelY = iy + (ITEM_H - tr.fontHeight) / 2
            ctx.drawText(tr, option, labelX, labelY, if (isChecked) accent else C_TEXT, false)
        }
        ctx.disableScissor()
