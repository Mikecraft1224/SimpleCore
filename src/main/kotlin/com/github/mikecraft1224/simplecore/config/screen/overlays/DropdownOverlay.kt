...
class DropdownOverlay(
    ...
    override fun render(state: GuiGraphicsExtractor, mx: Int, my: Int) {
        ...
        for (i in opts.indices) {
            ...
            when {
                isHov -> state.fill(px + 1, iy, px + popW - 1, iy + ITEM_H, C_SURFACE1)
                isSel -> state.fill(px + 1, iy, px + popW - 1, iy + ITEM_H, C_SURFACE0)
            }
            val col   = if (isSel) accent else C_TEXT
            val label = opts[i]
            val lw    = tr.width(label)
            state.text(tr, label, px + (popW - lw) / 2, iy + (ITEM_H - tr.lineHeight) / 2, col, false)
        }
