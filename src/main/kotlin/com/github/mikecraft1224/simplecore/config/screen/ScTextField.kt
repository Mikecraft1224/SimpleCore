...
/**
 * Minimal single-line text input that renders entirely inline via [GuiGraphicsExtractor].
 *
 * Unlike [net.minecraft.client.gui.components.EditBox] it is never registered as a
...
class ScTextField(
    ...
    fun render(state: GuiGraphicsExtractor, @Suppress("UNUSED_PARAMETER") _mx: Int, @Suppress("UNUSED_PARAMETER") _my: Int) {
        val tr    = Minecraft.getInstance().font
        val pad   = 3
        val textY = y + (h - tr.lineHeight) / 2

        state.fill(x, y, x + w, y + h - 1, 0xFF313244.toInt())
        val lineCol = if (focused) 0xFF89B4FA.toInt() else 0xFF45475A.toInt()
        state.fill(x, y + h - 1, x + w, y + h, lineCol)

        val innerW = w - pad * 2
        state.enableScissor(x + pad, y, x + pad + innerW, y + h)

        if (cursor != anchor) {
            val s = (tr.width(text.substring(0, selMin)) - scrollPx).coerceAtLeast(0)
            val e = (tr.width(text.substring(0, selMax)) - scrollPx).coerceAtMost(innerW)
            if (e > s) state.fill(x + pad + s, y + 1, x + pad + e, y + h - 2, 0x4489B4FA)
        }

        val textCol = if (focused) 0xFFCDD6F4.toInt() else 0xFF9399B2.toInt()
        state.text(tr, text, x + pad - scrollPx, textY, textCol, false)

        if (focused && (System.currentTimeMillis() / 530) % 2 == 0L) {
            val cx = x + pad + tr.width(text.substring(0, cursor)) - scrollPx
            state.fill(cx, y + 2, cx + 1, y + h - 2, 0xFFCDD6F4.toInt())
        }
    ...
    private fun copyToClipboard() {
        Minecraft.getInstance().keyboardHandler.clipboard = text.substring(selMin, selMax)
    }
    ...
    private fun charIndexAt(px: Int): Int {
        ...
        for (i in 1..text.length) {
            if (tr.width(text.substring(0, i)) > px) return i - 1
        }
        return text.length
    }
    ...
    private fun clampScroll() {
        val tr    = Minecraft.getInstance().font
        val inner = w - 6
        val curPx = tr.width(text.substring(0, cursor))
